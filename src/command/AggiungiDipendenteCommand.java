...
public class AggiungiDipendenteCommand implements Command {
    ...
    private JDialog apriFinestraDialogoComplessa(PannelloDisegno pd){
        ...
        ok.addActionListener(e -> {
            ...
            if(esisteDipendente(dipendente)){
    ...
    private boolean esisteDipendente(Dipendente dipendente){
        for(OrganigrammaElement elem: pd.getUnitaDisegnate()){
            Collection<Dipendente> listaDipendenti=elem.getDipendenti();
            for(Dipendente d: listaDipendenti){
                if(d.equals(dipendente))
                    return true;
            }
        }
        return false;
